...
var descripcion= descripcion.replace("nombre1",objetoRecuperado.name);
...
function sig_right() {
    ...
    var texto = valor === 0 ? `media/${objetoRecuperado.name}/${objetoRecuperado.name}.jpg` : `media/${objetoRecuperado.name}/${objetoRecuperado.name}.${valor}.jpg`;
    miImagen.src = texto;
}
